    ...
    IQueryHandler<GetOrderQuery, ErrorOr<OrderDto>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var orderId = validationResult.Value;

        var maybeOrder = await _orderRepository.GetByIdAsync(orderId, cancellationToken);

        if (maybeOrder.IsNull)
        {
            return ErrorOr<OrderDto>.WithError(OrderErrors.NotFound(orderId));
        }
